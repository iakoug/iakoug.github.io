...
published: true
cover: ./fireworks.jpg
coverAuthor: chriskwok
coverOriginalUrl: https://www.justwink.cn
---

从零开发一个类 Notion 的富文本编辑项目该怎么做？一个实现 Notion 富文本基本编辑功能的 demo 记录。

主要叙述数据结构的定义以及在定义的数据结构下的拖拽、光标、组合键等行为功能。
...
- 为文本直接子节点绑定 `onInput`、`onFocus`、`onBlur`、`onKeyUp`、`onKeyDown`、`contentEditable` 等事件以及属性
- 为定义结构支持拖拽的节点绑定 `onDrag`、`onDrop`、`onDragLeave`、`onDragOver`、`draggable` 等事件以及属性
...
deleteNode = (data, key) => {
  data.forEach((item, index) => {
    if (item.key === key) {
      console.log('[delete node match key]')
...
moveCursor = (id, trail = true) => {
  setTimeout(() => {
    ...
    console.log(dom, '[moveCursor] dom')
...
handleContentKeyDown = (item_, rowIndex_, itemIndex_) => e => {
  console.log('[handleContentKeyDown key code]', e.keyCode)
  ...
  if (e.keyCode === 8) {
    ...
    console.log('[FocusDivContent]', focusDivContent)

    // FIXME: 计算光标所在位置的前后文本之后退格操作会失效 光标立刻回到句首
    // const { before, after } = this.getCursorSplitTxt(cursorDOM)

    // 为空是再进行删除操作直接删除当前子元素光标移动至上一个紧邻元素
    // if (!before) {
  }
}
```
  ...
  const loop = item =>
    item.children.forEach((it, j) => {
      if (it.key === currentKey) {
        console.log('[Match findPreviousSiblingLastSubKey]: trigger sub')
...
handleContentKeyUp = (item, itemIndex, allItemsCount) => e => {
  ...
  if (e.keyCode === 8) {
    ...
    console.log('[Trigger backspace delete node & rending]', item.key)
...
handleContentKeyDown = (item_, rowIndex_, itemIndex_) => e => {
  ...
  if (e.keyCode === 13) {
    ...
    this.jsonRending(() => {
      // render 后延迟触发
      console.log(domId, '[handleContentKeyDown] new dom id')
      // 光标不需要移动至末尾
...
handleContentKeyDown = (item_, rowIndex_, itemIndex_) => e => {
  ...
  if (e.keyCode === 9) {
    ...
    // const rowNode = this.findNode(parentNode.parent).node
    // const isColumn =
    // parentNode.type === 'column' && rowNode.children.length === 1
...
至此文本块的基本行为大致 s 实现，最后再实现页面初始化的文本框，用于创建节点以及与已经创建的节点进行光标关联。
...
关于拖拽行为虽然做了排序处理但是并不完善，缺乏更精细化的行为，如拖拽到指定元素的指定位置（上、下）以及拖拽到右侧进行创建新列，具体实现的技术方案还没确定，预想大概是通过光标位置像素计算，后续更新。

2020.02.14 23:01:40 情人节快乐。

...
从零开发一个类 Notion 的富文本编辑项目该怎么做？使用最原始的方式实现 Notion 富文本基本编辑功能的 demo 记录。
...
handleContentKeyDown = (item_, rowIndex_, itemIndex_) => (e) => {
  ...
  if (e.keyCode === 13) {
    ...
    // 获取光标分割的文本
    const { before: originContentString, after: enterContentString } =
...
关于拖拽行为虽然做了排序处理但是并不完善，缺乏更精细化的行为，如拖拽到指定元素的指定位置（上、下）以及拖拽到右侧进行创建新列，由于不属于释放元素的监听区域具体实现的技术方案还没确定，预想是通过光标位置的元素计算。
